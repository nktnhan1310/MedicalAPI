...
namespace MrApp.API.Controllers
{
    ...
    public class UserFolderController : BaseController
    {
        ...
        /// <returns></returns>
        [HttpPut("update-user-file")]
        [MedicalAppAuthorize(new string[] { CoreContants.Update })]
        public async Task<AppDomainResult> UpdateUserFile([FromBody] UserFileRequestUpdateModel userFileRequestUpdateModel)
        {
            if (!ModelState.IsValid) throw new AppException(ModelState.GetErrorMessage());
            if (!userFileRequestUpdateModel.UpdateFolderId.HasValue || userFileRequestUpdateModel.UpdateFolderId.Value <= 0) throw new AppException("Vui lòng chọn thông tin folder");
            if (userFileRequestUpdateModel.UserFileIds == null || !userFileRequestUpdateModel.UserFileIds.Any()) throw new AppException("Vui lòng chọn thông tin file update");
            var successTask = this.userFileService.UpdateFolderForFile(userFileRequestUpdateModel.UserFileIds, userFileRequestUpdateModel.UpdateFolderId ?? 0);
            return new AppDomainResult()
            {
